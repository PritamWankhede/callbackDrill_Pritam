 /*
    ...
    Using callbacks and the fs module's asynchronous functions, do the following:
        ...
        3. Read the new file and convert it to lower case.if split the contents into sentences.if write it to a new file. Store the name of the new file in filenames.txt
...
function convertToUpperCase(data, filenames) {
    return new Promise((resolve, reject) => {
        if (!data) {
            reject(new Error('No data to convert'));
        } else {
        ...
        fs.readFile(newFileOfUpperCase, 'utf-8')
            .then(data => {
                if (data) {
                    ...
                    reject(new Error('No data to convert to lowercase and split.'));
                }
            })
            .catch(error => {
                console.error(`Error: ${error}`);
        ...
        fs.readFile(newsplitedFile, 'utf-8')
            .then(data => {
                if (data) {
                    ...
                    reject(new Error('No data found to sort.'));
                }
            })
            .catch(error => {
                console.error(`Error: ${error}`);
        ...
        fs.readFile(filenames, 'utf-8')
            .then(fileList => {
                if (fileList) {
                    ...
                    reject(new Error('No files to delete.'));
                }
            })
            .then(() => {
